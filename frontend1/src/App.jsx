...
const App = () => {
  ...
  useEffect(() => {
    ...
    axios.get('https://mern-app-2iv4.onrender.com/api/events')
  ...
  const handleEventDelete = (id) => {
    ...
    // Delete an event
    axios.delete(`https://mern-app-2iv4.onrender.com/api/events${id}`)
      .then(
        () =>
  ...
  const handleToggleReminder = (eventId) => {
    ...
    axios.put(`https://mern-app-2iv4.onrender.com/api/events${eventId}`, updatedEvent)
      .then(response => {
  ...
  const onEventEdit = (eventId, updatedData) => {
    // Update the event in the database
    axios.put(`https://mern-app-2iv4.onrender.com/api/events${eventId}`, updatedData)
      .then(response => {
        // If the update is successful, update the events in the state
        const updatedEvents = events.map(event =>
          event._id ===
